...
Route::middleware('auth')->group(function () {
    ...
    // PARTICIPATION MARKS (lecturer/admin only — authorization enforced in controller)
...
Route::middleware('auth')->group(function () {
    ...
    Route::prefix('quizzes')->name('quizzes.')->group(function () {
        ...
        // Performance report (lecturer/admin anytime; group members after the quiz closes)
    ...
    ->group(function () {
        ...
        Route::middleware(['system-admin'])->group(function () {
            ...
            Route::delete('/users/{user}', [UserManagementController::class, 'destroy'])
                ->name('users.destroy');
            Route::get('/users/{user}/reset-password', [UserManagementController::class, 'showResetPassword'])
                ->name('users.reset-password');
            Route::post('/users/{user}/reset-password', [UserManagementController::class, 'resetPassword'])
                ->name('users.reset-password.store');
            Route::get('/users/{user}/blacklist', [UserManagementController::class, 'showBlacklist'])
                ->name('users.blacklist');
            Route::post('/users/{user}/blacklist', [UserManagementController::class, 'blacklist'])
                ->name('users.blacklist.store');
            Route::post('/warnings/{warning}/resolve', [UserManagementController::class, 'resolveWarning'])
            ...
            // Admin Dashboard - All admins (includes platform statistics content)
            Route::get('/dashboard', function (StatisticsUtility $statisticsUtility) {
                return view('admin.dashboard', [
                    'groupStats' => $statisticsUtility->getStatsForUser(Auth::user()),
                ]);
            ...
            Route::middleware(['system-admin'])->group(function () {
                ...
                // Trashed groups & restore — System Admin only
                Route::get('/groups/trashed', [
                    GroupController::class,
                    'trashed',
                ])->name('admin.groups.trashed');
                Route::post('/groups/{group}/restore', [
                    GroupController::class,
                    'restore',
                ])
                    ->withTrashed()
