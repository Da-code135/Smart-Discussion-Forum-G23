...
class TopicController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $topic = Topic::create([
            ...
            "created_by" => $user->id,
            "group_id" =>
                $user->isSystemAdmin() && $request->has("group_id")
                    ? $validated["group_id"]
                    : $user->group_id,
            "status" => "active",
        ]);
    ...
    public function update(Request $request, int $topicId)
    {
        ...
        $validated = $request->validate([
            "title" =>
                "sometimes|string|max:255|unique:topics,title," . $topic->id,
    ...
    public function byType(Request $request, string $type)
    {
        if (!in_array($type, ["discussion", "question"])) {
            return response()->json(
                [
                    "message" =>
                        'Invalid type. Must be "discussion" or "question".',
                ],
                422,
            );
        }
    ...
    public function toggleAnswered(Request $request, int $topicId)
    {
        ...
        if ($topic->post_type !== "question") {
            return response()->json(
                [
                    "message" =>
                        "Only question topics can be marked as answered.",
                ],
                422,
            );
        }
        ...
        if ($topic->created_by !== $user->id && !$user->isAdmin()) {
            return response()->json(
                [
                    "message" =>
                        "You are not authorized to toggle answered status.",
                ],
                403,
            );
        }
